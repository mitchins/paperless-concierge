...
def require_authorization(func):
    ...
    async def wrapper(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        logger.info(f"Authorized user {user_id} (@{username}) accessing {func.__name__}")
...
class TelegramConcierge:
    ...
    async def handle_document(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # Generate a unique UUID for definitive tracking
        ...
        if message.photo:
            ...
            await message.reply_text("❌ Unsupported file type. Please send a photo or document.")
            return
        ...
        try:
            ...
            try:
                ...
                # Result can be either a string UUID or a dict with task_id
                ...
                if task_id:
                    ...
                    keyboard = [[
                        InlineKeyboardButton("🔄 Check Status", callback_data=f"status_{task_id}"),
                        InlineKeyboardButton("🔔 Notify When Done", callback_data=f"notify_{task_id}")
                    ]]
                    ...
                    await status_message.edit_text(f"✅ {original_filename} uploaded successfully!")
            ...
            try:
                ...
                if status.get('status') == 'SUCCESS':
                    await query.edit_message_text(
                        ...
                        f"📄 Ready in your Paperless-NGX instance."
                    )
                    ...
                    # Still processing
                    keyboard = [[InlineKeyboardButton("🔄 Check Again", callback_data=f"status_{task_id}")]]
                    reply_markup = InlineKeyboardMarkup(keyboard)
                    await query.edit_message_text(
                        f"⏳ Still processing...\n"
    ...
    @require_authorization
    async def query_documents(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        status_message = await update.message.reply_text(f"🔍 Searching for: {query_text}...")
        
        try:
            paperless_client = self.get_paperless_client(user_id)
            
            # First try AI query if available
            ai_response = await paperless_client.query_ai(query_text)
            
            if ai_response.get('success', False):
                # Format AI response with structured data
                ...
                if ai_response.get('confidence'):
                    confidence = round(float(ai_response['confidence']) * 100) if isinstance(ai_response['confidence'], (int, float)) else ai_response['confidence']
            ...
            elif ai_response.get('error') in ['AI service not configured', 'AI service temporarily unavailable']:
                # Fallback to regular search
                await status_message.edit_text(f"🔍 AI unavailable, searching documents...")
                
                search_results = await paperless_client.search_documents(query_text)
                
                if search_results.get('count', 0) > 0:
                    documents = search_results['results'][:5]  # Show top 5 results
                    ...
                    if search_results['count'] > 5:
                        response += f"... and {search_results['count'] - 5} more documents.\n"
                ...
                if search_results.get('count', 0) > 0:
                    ...
                    response = f"📋 Found {search_results['count']} documents (fallback search):\n\n"
                    for doc in documents:
            ...
            await status_message.edit_text(f"❌ Search failed: {str(e)}")
...
def main() -> None:
    ...
    application.add_handler(MessageHandler(filters.PHOTO | filters.Document.ALL, concierge.handle_document))
