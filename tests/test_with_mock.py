        ...
        with patch('config.TELEGRAM_BOT_TOKEN', mock_config['TELEGRAM_BOT_TOKEN']):
            with patch('config.PAPERLESS_URL', mock_config['PAPERLESS_URL']):
                with patch('config.PAPERLESS_TOKEN', mock_config['PAPERLESS_TOKEN']):
                    ...
                    required_methods = ['start', 'help_command', 'handle_document', 'query_documents', 'check_status', 'get_paperless_client']
                    
                    for method_name in required_methods:
                        assert hasattr(concierge, method_name), f"Missing method: {method_name}"
                        assert callable(getattr(concierge, method_name)), f"Method not callable: {method_name}"
...
async def test_message_formatting():
    ...
    for case_name, expected_prefix in test_cases:
